...
# import stuff 
import sys, time, pygame, json, keyboard, threading, traceback, urllib, os
...
def fetch_count():
    ...
    if not pfp_downloaded:
        urllib.request.urlretrieve(data_json["items"][0]["snippet"]["thumbnails"]["default"]["url"], "temp/profile_pic.png")
        
        pfp_img_size = data_json["items"][0]["snippet"]["thumbnails"]["default"]["width"], data_json["items"][0]["snippet"]["thumbnails"]["default"]["height"]
...
def main():
    ...
    while running:
        try:
            ...
            channel_str_Surf = fontNorm.render(channel_str, True, (255, 255, 255))
            apicount_str_Surf = fontSmol.render(f"API Calls: {str(api_calls)}", True, (255, 255, 255))
            
            screen.blit(count_str_Surf, (width / 2 - tw1 / 2, height / 2 + 10))
            screen.blit(channel_str_Surf, (width / 2 - tw2 / 2, height / 2 + 80))
            screen.blit(apicount_str_Surf, (10, height - 30))
            
            if not pfp_img is None:
                screen.blit(pfp_img, (width / 2 - pfp_img_size[0] / 2, height / 2 - 40 - pfp_img_size[1] / 2))
